...
//        twitterSource = new PlaybackTwitterSource(2.0);
    ...
    public void addQuery(Query query) {
        ...
        // connect the new query to the twitter source
    ...
    public Application() {
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                // set the text that appears at the mouse cursor
                List<MapMarker> markersCoveringList = getMarkersCovering(pos, pixelWidth(p));
                if (!markersCoveringList.isEmpty()) {
                    // show text
                    MapMarkerColor mapMarkerColor = (MapMarkerColor) markersCoveringList.get(0);
                    text = mapMarkerColor.getContent();
                    map().setToolTipText(text);
                }


            }
//          just on my local machine
            private void speechStatus(String text) {
                // speech text
                String textToSpeech = "";
                if (!lastText.equals(text)) {
                    // don't speech a text repeatedly a current cursor
                    if (text.contains("https")) {
                        textToSpeech = text.substring(0, text.lastIndexOf("https"));
                    }else{
                        textToSpeech = text;
                    }
                    System.out.println(textToSpeech);
                    ProcessBuilder pb = new ProcessBuilder("/usr/bin/en-speech.sh", textToSpeech);
                    try {
                        pb.start();
                        lastText = text;
                        System.out.println("text to speech is: " + textToSpeech);
    ...
    public void terminateQuery(Query query) {
        // disconnect the expiring query from the twitter source
